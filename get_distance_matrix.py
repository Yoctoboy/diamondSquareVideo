...
def get_distance_matrix(folder_name):
    file_names = sorted(
        [f for f in listdir(folder_name) if isfile(join(folder_name, f))]
    )
    ...
    for image_a_index, image_b_index in tqdm(
        list(combinations(range(files_amount), r=2)), desc="Computing distance matrix"
    ):
        image_a_name, image_b_name = (
            index_to_filenames[image_a_index],
            index_to_filenames[image_b_index],
        )
